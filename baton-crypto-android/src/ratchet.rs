...
impl RatchetState {
    ...
    fn kdf_rk(rk: &[u8; 32], dh_out: &[u8; 32]) -> ([u8; 32], [u8; 32]) {
        ...
        let mut okm = [0u8; 64];
        hk.expand(b"KDF_RK", &mut okm).unwrap();
    ...
    fn kdf_ck(ck: &[u8; 32]) -> ([u8; 32], [u8; 32]) {
        use hmac::{Hmac, Mac};
        let mut mac = <Hmac<Sha256> as Mac>::new_from_slice(ck).unwrap();
        mac.update(b"\x01");
        let mk = mac.finalize().into_bytes();

        let mut mac2 = <Hmac<Sha256> as Mac>::new_from_slice(ck).unwrap();
        mac2.update(b"\x02");
    ...
    /// Encrypt a payload.
    pub fn ratchet_encrypt(&mut self, plaintext: &[u8]) -> ([u8; 32], u32, u32, Vec<u8>) {
        let cks = self.cks.unwrap();
        ...
        (header_pub, header_n, header_pn, final_payload)
    }
